...
// char yazi[16] = "";
  ...
  lcd_print(1,1, "TIRSAN KARDAN");
  HAL_Delay(1000);
  lcd_clear();*/
  printf("ADC ile Seri Porttan Voltaj Okuma \n");
  HAL_Delay(1000);
 ...
 data = voltage;
 printf("Torque is: %-.2f Nm\n",  (data-7228.49)*1.4236);
     /*lcd_print(1,1, "Torque Is:");
     sprintf(yazi, "%-.2f", (data-7237.00)*1.3951);
     lcd_print(2,1, yazi);
     HAL_Delay(50);*/
   /* USER CODE BEGIN 3 */
  }
...
buffer[1] = 0xC2;
buffer[2] = 0x85;
HAL_I2C_Master_Transmit(&hi2c1, i2c_addr, (uint8_t*)buffer, 3, 100);


buffer[0] = 0x00;
HAL_I2C_Master_Transmit(&hi2c1, i2c_addr, (uint8_t*)buffer, 1, 100);
HAL_I2C_Master_Receive(&hi2c1, i2c_addr+1, buffer, 2, 100);
data = (buffer[0]<<8) + buffer[1];
if(data==0xFFFF) data = 0;

return (float)(0.5 * ((float)data));
